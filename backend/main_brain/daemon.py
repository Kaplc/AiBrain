...
class LifeLoopDaemon:
    ...
    # ── 意识流 tick（取代旧的 ActivitySelector→controller→gate）──
    ...
    def _maybe_consolidate(self, tick_type: str) -> None:
        """在 daily_tick 或 long_tick 时触发输出记忆沉淀（受开关控制）。

        daily_tick — 每天一次（传统路径，保留）。
        long_tick  — 约每小时一次，比纯等 daily_tick 更可靠（即使错过 daily 也不积压太久）。
        """
        try:
            ...
            logger.warning(f"[daemon] consolidate trigger ({tick_type}) failed: {e}")
