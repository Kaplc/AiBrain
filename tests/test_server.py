...
class TestServerTools:
    ...
    def test_store_tool_delegates_to_store_memory(self):
        with patch("mcp_qdrant.server.store_memory", return_value="已记住: hello") as mock_fn:
            from mcp_qdrant.server import store
    ...
    def test_search_tool_delegates_to_search_memory(self):
        ...
        with patch("mcp_qdrant.server.search_memory", return_value=fake_results) as mock_fn:
            from mcp_qdrant.server import search
    ...
    def test_list_all_tool_delegates_to_list_memories(self):
        ...
        with patch("mcp_qdrant.server.list_memories", return_value=fake_list) as mock_fn:
            from mcp_qdrant.server import list_all
            result = list_all()
            mock_fn.assert_called_once()
            assert result == fake_list

    def test_delete_tool_delegates_to_delete_memory(self):
        with patch("mcp_qdrant.server.delete_memory", return_value="已删除记忆: abc") as mock_fn:
            from mcp_qdrant.server import delete
            result = delete(memory_id="abc")
            mock_fn.assert_called_once_with("abc")
            assert result == "已删除记忆: abc"

    def test_mcp_instance_exists(self):
        from mcp_qdrant.server import mcp
