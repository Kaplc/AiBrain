...
const modelCard = overviewViewModel.modelCard
</script>
...
<template>
  <div class="status-card">
    <div class="sc-label-row">
      ...
      <span :class="modelCard.badgeClass()">
        <span v-if="modelCard.badge.value === 'loading'" class="mini-loading sm"></span>
        <template v-else>{{ modelCard.badge.value === 'ok' ? 'OK' : modelCard.badge.value }}</template>
      </span>
    </div>
    <div class="sc-sub">{{ modelCard.subText.value }}</div>
    <div class="sc-sub" v-if="modelCard.detail.value">{{ modelCard.detail.value }}</div>
  </div>
</template>
...
.mini-loading { display: inline-block; width: 12px; height: 12px; border: 2px solid #eab30844; border-top-color: #fde047; border-radius: 50%; animation: spin 0.8s linear infinite; }
